...
class ObjectController(BaseStorageServer):
    ...
    def _read_put_commit_message(self, mime_documents_iter):
        ...
        timeout_reader = self._make_timeout_reader(commit_iter)
        try:
            commit_body = ''.join(iter(timeout_reader, ''))
        except ChunkReadTimeout:
            raise HTTPClientDisconnect()
        return commit_body
        ...
        try:
            with disk_file.create(size=fsize) as writer:
                ...
                if use_multiphase_commit:
                    ...
                    print "send 100-continue"
                    commit_body = self._read_put_commit_message(
                        mime_documents_iter)
                    print "body %r" % commit_body
                    if commit_body.find("commit_confirmation") < 0:
                        print "no commit confirmation"
